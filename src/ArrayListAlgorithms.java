...
public class ArrayListAlgorithms {
    ...
    public static void replaceWithCaps(ArrayList<String> wordList) {
        for (int i = 0; i < wordList.size(); i++) {
            if (wordList.get(i).substring(wordList.get(i).length() - 1, wordList.get(i).length()).equals("s")) {
    ...
    public static int indexOfMinimum(ArrayList<Integer> intList) {
        ...
        for (int integer : intList) {
            if (integer < min) {
                min = integer;
                minIndex = intList.indexOf(integer);
            }
        }
        return minIndex;
    }
    ...
    public static ArrayList<String> parseWordsAndReverse(String sentence) {
        ...
        for (int i = 0; i < sentence.length(); i++) {
            if (sentence.substring(i, i + 1).equals(" ") || i == sentence.length() - 1) {
                stringArrayList.add(0, sentence.substring(lastSpace + 1, i + 1));
                lastSpace = i;
            }
        }
    ...
    public static ArrayList<Integer> modes(int[] numList) {
        ...
        for(int i = 0 ; i < numList.length; i++) {
            ...
            if(tempCount > modeCount && tempCount > 1) {
                integerArrayList.clear();
            }
            if(!(tempCount < modeCount) && tempCount > 1) {
                integerArrayList.add(check);
                modeCount = tempCount;
            }
        }
